...
async function syncEventbrite(event_id) {
  ...
  for (const eb of attendees) {
    ...
    const name = `${profile.first_name || ''} ${profile.last_name || ''}`.trim() || 'Guest';
    const email = profile.email?.toLowerCase();
    const eventbrite_id = String(eb.id);
    if (!email) {
      console.log(`[sync] Skipping attendee ${eventbrite_id} - no email`);
      continue;
    }
    ...
    for (const answer of answers) {
      const question = (answer.question?.text || answer.question?.name || answer.question || '').toLowerCase().trim();
      const value = (answer.answer?.text || answer.answer || answer.text || '').toString().trim() || null;
      if (!value) continue;
      if (question.includes('organization') || question.includes('company')) { company = value; }
      if (question.includes('position within') || question.includes('job title')) { title = value; }
    }
    // If company and title are identical, the title got put in company — clear company
    if (company && company === title) { company = null; }
    ...
    if (existing) {
      if (existing.manually_edited) {
        ...
        // Only overwrite company/title if we have new values from answers
...
async function fetchAllAttendees(access_token, eventbrite_event_id) {
  ...
  while (hasMore) {
    const response = await axios.get(`https://www.eventbriteapi.com/v3/events/${eventbrite_event_id}/attendees/`, {
      ...
      params: { page, status: 'attending', expand: 'attendees.profile,attendees.answers,attendees.barcodes' }
    });
