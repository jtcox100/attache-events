...
const FROM_NAME = process.env.RESET_FROM_NAME || 'Attache Events';
...
router.post('/request', async (req, res) => {
  ...
  try {
    let query = supabase.from('attendees').select('id, name, email, event_id').eq('email', email.toLowerCase().trim());
    if (event_id) query = query.eq('event_id', event_id);
    const { data: attendees } = await query;

    // Always return success to prevent email enumeration
    if (!attendees?.length) return res.json({ message: 'If that email is registered, you will receive a reset link shortly' });

    // Send reset for all matching accounts (handles same email across multiple events)
    ...
    const result = await sendEmail(
      ...
      `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 2rem;">
          ...
          <p style="color: #374151; margin: 0 0 1.5rem;">We received a request to reset your password for the event app. Click the button below to set a new password.</p>
          ...
          <p style="color: #9ca3af; font-size: 12px; margin: 0;">Attache Group Inc.</p>
        </div>
      `
    );
