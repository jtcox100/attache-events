...
// Copy vendors from one event to another
router.post('/copy-to-event', authenticate, requireAdmin, async (req, res) => {
  const { from_event_id, to_event_id } = req.body;
  if (!from_event_id || !to_event_id) return res.status(400).json({ error: 'from_event_id and to_event_id required' });
  try {
    // Get all vendors from source event
    const { data: vendors, error } = await supabase.from('vendors').select('*').eq('event_id', from_event_id);
    if (error) throw error;
    if (!vendors?.length) return res.json({ copied: 0, message: 'No vendors found in source event' });

    let copied = 0, skipped = 0;
    for (const v of vendors) {
      // Check if vendor already exists in target event with same email
      const { data: existing } = await supabase.from('vendors')
        .select('id').eq('email', v.email).eq('event_id', to_event_id).single();
      if (existing) { skipped++; continue; }
      // Copy vendor with new event_id
      await supabase.from('vendors').insert({
        email: v.email, password_hash: v.password_hash,
        company_name: v.company_name, event_id: to_event_id
      });
      copied++;
    }
    res.json({ copied, skipped, message: `${copied} vendor(s) copied, ${skipped} already existed` });
