    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-6 py-6">
        ...
        <div className="bg-white rounded-xl border p-6" style={{ borderColor: '#D5D5D4' }}>
          ...
          <div className="space-y-3">
            {sent.map(m => (
              <div key={m.id} className="flex items-start justify-between gap-3 p-3 rounded-lg" style={{ backgroundColor: '#f9fafb', border: '1px solid #e5e7eb' }}>
                <div className="flex-1">
                  <p className="text-sm text-gray-900">{m.message}</p>
                  <p className="text-xs text-gray-400 mt-1">✓ Sent {formatTime(m.sent_at)}</p>
                </div>
                <button onClick={() => handleDelete(m.id)}
                  className="text-xs px-2 py-1 border rounded shrink-0" style={{ borderColor: '#9D2235', color: '#9D2235', cursor: 'pointer', background: 'transparent' }}>
                  Delete
                </button>
              </div>
            ))}
