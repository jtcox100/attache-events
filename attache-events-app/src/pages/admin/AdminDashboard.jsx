...
export default function AdminDashboard() {
  ...
  const [badgeShowTitle, setBadgeShowTitle] = useState(true);
  const [additionalBadges, setAdditionalBadges] = useState([{ name: '', partner_id: '' }]);
  ...
  useEffect(() => {
    // A selection sets the query to the attendee's name, which re-fires this effect.
    // Skip that one run so it doesn't repopulate the dropdown or clear the selected card.
  ...
  function selectAttendee(a) { skipNextSearch.current = true; setSearchResult(a); setSearchResults([]); setSearchQuery(a.name); }
  ...
  async function handleCopyVendors() {
    const events = await api.get('/api/events').then(r => r.data || []);
    const otherEvents = events.filter(e => e.id !== eventId);
    if (!otherEvents.length) { alert('No other events found to copy to'); return; }
    const targetEvent = otherEvents.length === 1 ? otherEvents[0] : null;
    const targetName = targetEvent?.name || otherEvents.map(e => e.name).join(', ');
    if (!window.confirm(`Copy all vendors from this event to "${targetName}"? Vendors already registered for that event will be skipped.`)) return;
    try {
      const { data } = await api.post('/api/vendor/copy-to-event', {
        from_event_id: eventId,
        to_event_id: targetEvent?.id || otherEvents[0].id
      });
      alert(data.message);
    } catch (err) { alert(err.response?.data?.error || 'Failed to copy vendors'); }
  }

  async function handlePartnerBadges() {
    if (!eventId) { alert('Select an event first'); return; }
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/badges/all-partner/${eventId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      if (!res.ok) { alert('Failed to generate partner badges'); return; }
      const blob = await res.blob();
      const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
      a.download = 'partner-badges.pdf'; a.click();
    } catch (err) { alert('Error generating partner badges'); }
  }
  ...
  async function handleBadgeExport() {
    const token = localStorage.getItem('token');
    const url = `${import.meta.env.VITE_API_URL}/api/badges/event/${eventId}?show_title=${badgeShowTitle}`;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="mb-6 flex flex-wrap items-center gap-3">
          ...
          <a href={`/admin/attendee-snapshot/${eventId}`} className={BTN} style={{ ...BTN_TEAL, textDecoration: 'none' }}>Attendee Snapshot</a>
          ...
          <button onClick={() => navigate('/admin/exhibitors')} className={BTN} style={BTN_TEAL}>Exhibitors</button>
          <button onClick={() => navigate('/admin/qrcode')} className={BTN} style={BTN_TEAL}>Event QR Code</button>
          ...
          <a href="/admin/single-badge" className={BTN} style={{ ...BTN_TEAL, textDecoration: 'none' }}>Ad-Hoc Badges</a>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            ...
            <label style={{ fontSize: 12, color: '#6b7280', display: 'flex', alignItems: 'center', gap: 4, cursor: 'pointer' }}>
              <input type="checkbox" checked={badgeShowTitle} onChange={e => setBadgeShowTitle(e.target.checked)} />
              Include title
            </label>
          </div>
          <button onClick={handlePartnerBadges} className={BTN} style={BTN_DARK}>Partner Badges</button>
          <button onClick={handleCopyVendors} className={BTN} style={{ ...BTN_DARK, backgroundColor: '#185676' }}>Copy Vendors to Other Event</button>
        ...
        {/* Additional partner badges panel */}
