...
export default function AdminSetup() {
  ...
  const [selectedEvent, setSelectedEvent] = useState(user?.event_id || '');
  ...
  const emptySession = { room_id: '', title: '', description_brief: '', description_full: '', track: '', start_time: '', end_time: '', is_mandatory: false };
  ...
  async function handleSessionSubmit(e) {
    ...
    const primary = selectedSpeakers[0] || null;
    const payload = { room_id: sessionForm.room_id, title: sessionForm.title, description_brief: sessionForm.description_brief || null, description_full: sessionForm.description_full || null, speaker_id: primary, track: sessionForm.track || null, start_time: sessionForm.start_time + ':00-04:00', end_time: sessionForm.end_time + ':00-04:00', is_mandatory: sessionForm.is_mandatory };
    try {
      let sessionId;
      if (editingSession) {
        await api.put(`/api/sessions/${editingSession.id}`, payload);
        sessionId = editingSession.id;
        showMsg('Session updated');
      } else {
        const { data } = await api.post('/api/sessions', { ...payload, event_id: selectedEvent });
        sessionId = data?.id || data?.session_id;
        showMsg('Session added');
      }
      // Save speaker assignments
      if (sessionId) {
        await api.put(`/api/sessions/${sessionId}/speakers`, { speaker_ids: selectedSpeakers });
      }
      setSessionForm(emptySession); setEditingSession(null); setSelectedSpeakers([]); loadSessions();
    } catch (err) { showError(err.response?.data?.error || 'Failed'); }
  }
  ...
  function startEditSession(s) {
    ...
    setSessionForm({ room_id: s.room_id || '', title: s.title || '', description_brief: s.description_brief || '', description_full: s.description_full || '', track: s.track || '', start_time: toLocalInput(s.start_time), end_time: toLocalInput(s.end_time), is_mandatory: s.is_mandatory || false });
    // Load existing speaker assignments
    api.get(`/api/sessions/${s.session_id || s.id}/speakers`).then(r => {
      setSelectedSpeakers((r.data || []).map(sp => sp.id));
    }).catch(() => setSelectedSpeakers(s.speaker_id ? [s.speaker_id] : []));
  ...
  function parseBulk(text) {
    ...
    dataLines.forEach((line, i) => {
      ...
      const room = rooms.find(r => r.name.toLowerCase().trim() === (roomName||'').toLowerCase().trim());
      if (!room) {
        console.log('[bulk] room not found:', JSON.stringify(roomName), 'available:', rooms.map(r => JSON.stringify(r.name)));
        errors.push(`Row ${lineNum}: room "${roomName}" not found`); return;
      }
  ...
  async function handleSwapRooms() {
    if (swapSelection.length !== 2) { setSwapMsg('Select exactly 2 sessions to swap'); return; }
    if (!window.confirm(`Swap rooms between "${swapSelection[0].title}" and "${swapSelection[1].title}"? A broadcast notification will be sent to all attendees.`)) return;
    try {
      const { data } = await api.post('/api/sessions/swap-rooms', {
        session_id_a: swapSelection[0].session_id || swapSelection[0].id,
        session_id_b: swapSelection[1].session_id || swapSelection[1].id
      });
    ...
    } catch (err) { setSwapMsg(err.response?.data?.error || 'Swap failed'); }
  }

  function toggleSwapSelection(session) {
    setSwapSelection(prev => {
      const exists = prev.find(s => (s.session_id || s.id) === (session.session_id || session.id));
      if (exists) return prev.filter(s => (s.session_id || s.id) !== (session.session_id || session.id));
              ...
              <div>
                {sessions.length > 0 && (
                  <div className="flex justify-end gap-2 mb-4 flex-wrap">
                    {swapMsg && <p style={{ fontSize: 12, color: '#2d6a2d', width: '100%', textAlign: 'right' }}>{swapMsg}</p>}
                    {swapMode ? (
                      <>
                        <span style={{ fontSize: 12, color: '#6b7280', alignSelf: 'center' }}>
                          {swapSelection.length === 0 ? 'Select first session' : swapSelection.length === 1 ? 'Select second session' : 'Ready to swap'}
                        </span>
                        <button onClick={handleSwapRooms} disabled={swapSelection.length !== 2}
                          className="text-xs px-3 py-1.5 rounded-lg font-medium text-white"
                          style={{ backgroundColor: swapSelection.length === 2 ? '#185676' : '#9ca3af', border: 'none', cursor: swapSelection.length === 2 ? 'pointer' : 'default' }}>
                          Confirm Swap
                        </button>
                        <button onClick={() => { setSwapMode(false); setSwapSelection([]); }}
                          className="text-xs px-3 py-1.5 rounded-lg font-medium"
                          style={{ border: '1px solid #D5D5D4', color: '#6b7280', backgroundColor: 'transparent', cursor: 'pointer' }}>
                          Cancel
                        </button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => setSwapMode(true)}
                          className="text-xs px-3 py-1.5 rounded-lg font-medium"
                          style={{ border: '1px solid #185676', color: '#185676', backgroundColor: 'transparent', cursor: 'pointer' }}>
                          Swap Rooms
                        </button>
                ...
                <div className="bg-white rounded-xl border p-6" style={BORDER}>
                  ...
                  <form onSubmit={handleSessionSubmit} className="space-y-3">
                    <div><label className="block text-sm font-medium text-gray-700 mb-1">Room</label>
                      <select value={sessionForm.room_id} onChange={e => {
                          const selectedRoom = rooms.find(r => r.id === e.target.value);
                          setSessionForm(p => ({ ...p, room_id: e.target.value, track: selectedRoom?.track || p.track || '' }));
                        }} required className={INPUT} style={BORDER}>
                    ...
                    <div><label className="block text-sm font-medium text-gray-700 mb-1">Track <span className="text-gray-400 font-normal">(optional — overrides room track)</span></label>
                      <input type="text" value={sessionForm.track} onChange={e => setSessionForm(p => ({ ...p, track: e.target.value }))} className={INPUT} style={BORDER} placeholder="e.g. Demonstration, Artificial Intelligence" /></div>
                    <div><label className="block text-sm font-medium text-gray-700 mb-1">Speakers</label>
                      {speakers.length === 0 ? (
                        <p className="text-xs text-gray-400">No speakers added yet — go to the Speakers page first</p>
                      ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 160, overflowY: 'auto', border: '1px solid #D5D5D4', borderRadius: 8, padding: '0.5rem' }}>
                          {speakers.map(sp => (
                            <label key={sp.id} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13,
                              padding: '4px 6px', borderRadius: 6,
                              backgroundColor: selectedSpeakers.includes(sp.id) ? '#fdf0f2' : 'transparent',
                              border: `1px solid ${selectedSpeakers.includes(sp.id) ? '#9D2235' : 'transparent'}` }}>
                              <input type="checkbox" checked={selectedSpeakers.includes(sp.id)}
                                onChange={e => setSelectedSpeakers(prev => e.target.checked ? [...prev, sp.id] : prev.filter(id => id !== sp.id))}
                                style={{ accentColor: '#9D2235' }} />
                              {sp.photo_url && <img src={sp.photo_url} alt={sp.name} style={{ width: 24, height: 24, borderRadius: '50%', objectFit: 'cover' }} />}
                              <span>{sp.name}{sp.company ? ` — ${sp.company}` : ''}</span>
                            </label>
                          ))}
                        </div>
                      )}
                    ...
                    <div key={track} className="mb-4">
                      ...
                      <div className="space-y-2">
                        {[...ts].sort((a,b) => new Date(a.start_time)-new Date(b.start_time)).map(s => {
                          ...
                          return (
                            <div key={sid}
                              onClick={swapMode ? () => toggleSwapSelection(s) : undefined}
                              className="p-3 border rounded-lg"
                              style={{
                                borderColor: swapMode && swapSelection.find(x => (x.session_id||x.id) === sid) ? '#185676' : '#f0f0f0',
                                backgroundColor: swapMode && swapSelection.find(x => (x.session_id||x.id) === sid) ? '#f0f5fa' : '#fff',
                                cursor: swapMode ? 'pointer' : 'default'
                              }}>
                              <div className="flex items-start justify-between">
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm font-medium truncate" style={{ color: '#262D33' }}>
                                    {swapMode && swapSelection.find(x => (x.session_id||x.id) === sid) ? '✓ ' : ''}{s.title}{s.is_mandatory ? ' ★' : ''}
                                  </p>
