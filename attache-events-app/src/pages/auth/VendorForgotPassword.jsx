...
export default function VendorForgotPassword() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      await api.post('/api/password/vendor/request', { email });
      setSubmitted(true);
    } catch (err) {
        ...
        <div style={{ backgroundColor: '#fff', borderRadius: 12, border: '1px solid #D5D5D4', padding: '2rem' }}>
          {submitted ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <Link to="/manage" style={{ fontSize: 14, color: '#9D2235', fontWeight: 600, textDecoration: 'none' }}>← Back to sign in</Link>
            </div>
          ) : (
            <>
              <h2 style={{ fontSize: 18, fontWeight: 700, color: '#262D33', margin: '0 0 0.5rem' }}>Reset your password</h2>
              <p style={{ fontSize: 14, color: '#6b7280', margin: '0 0 1.5rem' }}>Enter your vendor email and we'll send a reset link.</p>
              ...
              <p style={{ textAlign: 'center', fontSize: 13, color: '#6b7280', marginTop: '1rem' }}>
                <Link to="/manage" style={{ color: '#9D2235', fontWeight: 600, textDecoration: 'none' }}>← Back to sign in</Link>
              </p>
            </>
          )}
        </div>
