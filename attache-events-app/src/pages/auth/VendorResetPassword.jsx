...
export default function VendorResetPassword() {
  ...
  useEffect(() => {
    ...
    api.get(`/api/password/vendor/validate/${token}`)
  ...
  async function handleSubmit(e) {
    ...
    try {
      await api.post('/api/password/vendor/reset', { token, password });
      setSuccess(true);
    } catch (err) {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '1rem', backgroundColor: '#f5f5f5' }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <p style={{ color: '#6b7280', fontSize: 14, margin: 0 }}>Vendor Portal</p>
        </div>
        <div style={{ backgroundColor: '#fff', borderRadius: 12, border: '1px solid #D5D5D4', padding: '2rem' }}>
          ...
          {tokenValid === false && (
            <div style={{ textAlign: 'center' }}>
              ...
              <Link to="/vendor/forgot-password" style={{ display: 'inline-block', padding: '10px 20px', borderRadius: 8, backgroundColor: '#9D2235', color: '#fff', textDecoration: 'none', fontSize: 14, fontWeight: 600 }}>
                Request a new link
              </Link>
            </div>
          )}
          {tokenValid === true && success && (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 14, color: '#6b7280', margin: '0 0 1.5rem' }}>Your vendor password has been changed.</p>
              <Link to="/manage" style={{ display: 'inline-block', padding: '10px 20px', borderRadius: 8, backgroundColor: '#9D2235', color: '#fff', textDecoration: 'none', fontSize: 14, fontWeight: 600 }}>
                Sign in
              </Link>
            </div>
          )}
          {tokenValid === true && !success && (
            <>
              ...
              <p style={{ fontSize: 14, color: '#6b7280', margin: '0 0 1.5rem' }}>Choose a new password for your vendor account.</p>
