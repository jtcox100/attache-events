...
export default function ForgotPassword() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      await api.post('/api/password/request', { email, event_id: eventId || undefined });
      setSubmitted(true);
    } catch (err) {
        ...
        <div style={{ backgroundColor: '#fff', borderRadius: 12, border: '1px solid #D5D5D4', padding: '2rem' }}>
          {submitted ? (
            ...
            <>
              ...
              <p style={{ fontSize: 14, color: '#6b7280', margin: '0 0 1.5rem' }}>Enter your email and we'll send you a reset link.</p>
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {events.length > 1 && (
                  <div>
                    <label style={{ display: 'block', fontSize: 14, fontWeight: 500, marginBottom: 4 }}>Event <span style={{ color: '#9ca3af', fontWeight: 400 }}>(optional)</span></label>
