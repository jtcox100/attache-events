...
function getTrackStyle(track, isMandatory) {
  ...
  if (t.includes('traditional'))   return { bg: '#f8f8f9', badge: '#4A5568', text: '#fff' };
  if (t.includes('demonstration')) return { bg: '#eff6ff', badge: '#2563EB', text: '#fff' };
  if (t.includes('artificial'))    return { bg: '#ecfdf5', badge: '#059669', text: '#fff' };
  if (t.includes('cyber'))         return { bg: '#f5f3ff', badge: '#7C3AED', text: '#fff' };
...
export default function AttendeeSchedule() {
  ...
  return (
    <div style={{ backgroundColor: bg, paddingBottom: 90 }}>
      {/* Header */}
      <div style={{ backgroundColor: dark ? '#1e293b' : '#ffffff', borderBottom: `1px solid ${border}`, padding: '1rem 1.25rem 0', position: 'sticky', top: 0, zIndex: 20, paddingTop: 'env(safe-area-inset-top)' }}>
            ...
            <div key={timeKey} style={{ marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {timeSessions.map(s => {
                  ...
                  const capacityStatus = sessionStatus(s);
                      ...
                      <div style={{ padding: '0.75rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            {s.additional_speakers && <p style={{ fontSize: 12, color: sub, margin: '0 0 4px' }}>{s.additional_speakers}</p>}
                            {s.track && (
                        ...
                        <div style={{ marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: `1px solid ${border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          {isPast ? (
                            ...
                            <span style={{ fontSize: 12, color: capacityStatus.color, fontWeight: capacityStatus.key === 'available' ? 400 : 600 }}>{capacityStatus.label}</span>
                          )}
