...
export default function AttendeeSpeakerDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [spRes, sessRes, ssRes] = await Promise.all([
          api.get(`/api/speakers/event/${user.event_id}`),
          api.get(`/api/sessions/event/${user.event_id}`),
          api.get(`/api/speakers/${id}/sessions`)
        ]);
        const found = (spRes.data || []).find(s => s.id === id);
        setSpeaker(found || null);
        const sessionIds = new Set();
        (sessRes.data || []).forEach(s => {
          if (s.speaker_id === id) sessionIds.add(s.session_id || s.id);
        });
        (ssRes.data || []).forEach(sid => sessionIds.add(sid));
        const spSessions = (sessRes.data || []).filter(s => sessionIds.has(s.session_id || s.id));
