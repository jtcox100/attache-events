...
export default function AttendeeHome() {
  ...
  return (
    <div style={{ paddingBottom: 90 }}>
      {/* Header */}
      <div style={{ backgroundColor: user?.header_color || event?.header_color || '#9D2235', padding: '0', position: 'relative' }}>
        <div style={{ padding: '0.6rem 0.75rem 1rem', position: 'relative', paddingTop: 'calc(env(safe-area-inset-top) + 0.6rem)' }}>
          ...
          <div style={{ lineHeight: 0, fontSize: 0 }}>
            {event && <img src={event.badge_logo_url || "/showcase-logo-white.png"} alt={event.name || "London Technology Showcase"}
              style={{ width: '100%', maxWidth: 320, height: 'auto', maxHeight: 140, objectFit: 'contain', display: 'block', margin: '0 auto', border: 'none', outline: 'none' }} />}
          </div>
        </div>
        ...
        {tiles.map(tile => (
          <button key={tile.path} onClick={() => navigate(tile.path)}
            style={{
              ...
              borderRadius: 12, padding: '1rem 1rem',
